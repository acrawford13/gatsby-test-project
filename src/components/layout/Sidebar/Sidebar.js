...
const Sidebar = ({ sidebar, isOpen, setIsOpen }) => {
  const sidebarHeadings = sidebar.body.filter(item => item.primary);
            ...
            <ul>
              {sidebarHeadings &&
                sidebarHeadings.map(heading => (
                  <li
                    key={heading.id}
                    className={`sidebar__list-item sidebar__list-item--${
                      heading.slice_type.match(/^level_(\d)_heading$/)[1]
                    }`}
                  >
                    <a
                      onClick={() => setIsOpen(false)}
                      href={'#' + heading.primary.heading.text.toLowerCase().replace(/\s/g, '-')}
                    >
